import { Table, TableHead, TableHeadCell, Checkbox, TableBody, TableRow, TableCell, Pagination, Button, Modal } from 'flowbite-react';
...
const Users: React.FC = () => {
  ...
  const deleteUser = (id: string) => {
    axios.delete(`${import.meta.env.VITE_BASE_URL}/users/${id}`).then((res: any) => {
      getUsers();
      setOpenModal(false);
      setPage(1)
    })
        ...
        {currentItems.map((item: User, index: number) => (
          ...
          <TableCell className="p-4">
            <Checkbox onChange={(e) => {       
              ...
              if(e.target.checked) {
                setCheckedUser([...checkedUser, item])
              } else {
                const i = checkedUser.indexOf(item);
                const u = checkedUser.splice(i, 1);
          ...
          <TableCell>
            ...
            <div className="flex justify-around">
              <Link to={item.id} className="font-medium text-cyan-600 hover:underline dark:text-cyan-500">
                {t('view')}
              </Link>
              <Link to={`${item.id}/edit`} className="font-medium text-cyan-600 hover:underline dark:text-cyan-500">
                {t('edit')}
              </Link>
              <button onClick={() => confirmDelete(item.id)} className="font-medium text-cyan-600 hover:underline dark:text-cyan-500">
                {t('delete')}
              </button>
            </div>
        ...
        <Modal.Body>
          <div className="text-center">
            ...
            <div className="flex justify-center gap-4">
              <Button color="failure" onClick={() => idDelete && deleteUser(idDelete)}>
                {"Yes, I'm sure"}
              </Button>
